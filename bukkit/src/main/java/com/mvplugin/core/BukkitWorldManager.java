...
public class BukkitWorldManager extends AbstractWorldManager {
    ...
    public BukkitWorldManager(@NotNull final MultiverseCorePlugin plugin) {
        super(plugin);
        this.plugin = plugin;
    ...
    private void initializeDefaultWorldGenerators() {
        ...
        if (files != null && files.length == 1) {
            ...
            Logging.warning("Could not read 'bukkit.yml'. Any Default world generators will not be loaded!");
        }
    }
    ...
    private void initializeWorlds() {
        ...
        for (final World w : this.plugin.getServer().getWorlds()) {
            try {
                worldsMap.put(w.getName(), newBukkitWorld(w));
                if (builder.length() != 0) {
                    builder.append(", ");
                }
    ...
    public BukkitMultiverseWorld createWorld(@NotNull final WorldCreationSettings settings) throws WorldCreationException {
        final World world = Bukkit.getWorld(settings.name());
        if (world != null) {
            if (isMVWorld(world.getName())) {
                // TODO: return existing MV World when we can get it
            }
            try {
                return newBukkitWorld(world);
            } catch (Exception e) {
        ...
        try {
            ...
            return newBukkitWorld(w);
        } catch (Exception e) {
            throw new WorldCreationException(new BundledMessage(BukkitLanguage.CREATE_WORLD_ERROR, settings.name()), e);
        }
    }

    private BukkitWorld newBukkitWorld(@NotNull final World world) throws IOException {
